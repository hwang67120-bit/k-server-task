...
1. 사용자가 전체 메뉴 목록을 요청한다.
2. 시스템은 메뉴 ID, 카테고리 ID·이름, 메뉴 이름과 가격을 반환한다.
3. 사용자는 주문할 메뉴를 선택한다.
4. 사용자가 메뉴 ID로 상세 정보를 요청한다.
5. 시스템은 메뉴 기본 정보와 사이즈, 온도, 원두, 추가 가격, 시럽 가능 여부를 포함한 선택 옵션을 반환한다.
...
5. 포인트 차감, 주문 생성과 포인트 사용 이력 저장을 하나의 트랜잭션으로 처리한다.
6. 모든 작업이 성공하면 커밋하고 하나라도 실패하면 전체 롤백한다.
7. 주문 트랜잭션이 커밋된 뒤 OrderEventProducer가 Kafka로 주문 완료 이벤트를 전달한다.
...
- 메뉴 옵션의 가격은 기본 가격에 더하는 원화 단위 추가 금액이며 `BIGINT`와 Java `long`으로 관리한다.
- 메뉴 사이즈는 `REGULAR`, `LARGE`, 온도는 `HOT`, `ICE` 상태값만 허용한다.
- 원두 종류는 문자열로 관리하고 시럽 가능 여부는 boolean 상태값으로 관리한다.
- 포인트 잔액은 음수가 될 수 없다.
...
- 주문 저장, 포인트 차감과 포인트 내역 저장은 모두 성공하거나 모두 롤백한다.
- 주문 트랜잭션이 커밋된 뒤에만 Kafka 주문 완료 이벤트를 발행한다.
...
| 메뉴 상세 조회 | 요청한 메뉴 ID가 없음 | `404 Not Found`와 `조회된 메뉴가 없습니다.` 반환 | 변경 없음 | 메뉴 ID 확인 후 가능 |
...
API 경로와 오류 형식은 구현 과정에서 조정할 수 있다. 주문 멱등성 키는 현재 요청 DTO의 `orderToken`으로 전달하고, 포인트 충전은 `paymentId`를 결제 멱등성 키로 사용한다.
...
옵션이 없는 메뉴는 `options`를 빈 배열로 반환한다. 최종 선택 가격은 메뉴의 `price`와 옵션의 `additionalPrice`를 더해 계산한다.
...
주문 결제가 커밋되면 OrderCompletedEvent(eventId, productId, quantity, orderedAt)를 order-events Kafka 토픽으로 발행한다. Consumer는 eventId를 먼저 원자적으로 등록하고, 처리에 성공한 이벤트만 일별 판매 집계에 반영한다.
...
| 오류 코드 | HTTP 상태 | 메시지 |
|---|---:|---|
| `PAYMENT_REJECTED` | 409 | 결제가 거부되었습니다. |
| `ORDER_FAILED` | 500 | 주문에 실패했습니다. |
| `DUPLICATE_REQUEST` | 409 | 중복된 요청입니다. |
| `POINT_CHARGE_FAILED` | 500 | 충전에 실패했습니다. |
| `MENU_NOT_FOUND` | 404 | 조회된 메뉴가 없습니다. |
| `MENU_CATEGORY_MISSING` | 500 | 카테고리가 누락되었습니다. |

`MENU_NOT_FOUND`는 특정 메뉴 ID를 조회하지 못한 경우에 사용한다. 메뉴 목록이 비어 있는 경우에는 기존 정책대로 정상 응답과 빈 목록을 반환한다.
...
erDiagram
    ...
    MENU_CATEGORY ||--o{ MENU : "메뉴 분류"
    MENU ||--o{ COFFEE_ORDER : "주문 대상"
    MENU ||--o{ MENU_OPTION : "선택 옵션 제공"
    COFFEE_ORDER o|--o| POINT_HISTORY : "포인트 사용 근거"
    COFFEE_ORDER ||--o{ DAILY_PRODUCT_SALES : "완료 이벤트 집계"
    ...
    MENU_OPTION {
        ...
        VARCHAR(100) bean_type
        BIGINT additional_price
        BOOLEAN syrup_available
        BIGINT version
        DATETIME created_at
        DATETIME updated_at
    }
...
- `MENU_CATEGORY.name` UNIQUE
- `POINT_HISTORY.idempotency_key` UNIQUE
- `MENU_OPTION.additional_price >= 0`
- `MENU_OPTION.size IN (REGULAR, LARGE)`
- `MENU_OPTION.temperature IN (HOT, ICE)`
- `COFFEE_ORDER.idempotency_key` UNIQUE
- `OUTBOX_EVENT.order_id` UNIQUE
- `PROCESSED_EVENT.event_id` PRIMARY KEY
- `DAILY_PRODUCT_SALES(product_id, sales_date)` UNIQUE
- `DAILY_PRODUCT_SALES.quantity >= 0`
- `DAILY_PRODUCT_SALES.order_count >= 0`
- 인기 메뉴 집계를 위한 주문 인덱스: `(payment_status, paid_at, menu_id)`
- Outbox 선점 대상을 찾기 위한 인덱스: `(status, next_retry_at, claimed_until)`

감사 시간이 필요한 엔티티는 별도 테이블을 만들지 않는 `BaseTimeEntity`를 상속하여 `created_at`과 `updated_at`을 공통으로 기록한다. `DAILY_PRODUCT_SALES`는 일별 집계 전용 테이블이므로 현재는 업무 집계 컬럼만 저장한다. `paid_at`, `sent_at`, `processed_at`처럼 업무 사건의 발생 시각은 공통 감사 시간과 구분하여 유지한다.
...
메뉴 카테고리는 enum이 아니라 `MENU_CATEGORY` 데이터로 관리하여 새로운 카테고리를 코드 배포 없이 추가할 수 있게 한다. 하나의 메뉴는 하나의 카테고리 ID를 참조한다. 이벤트 상품이 기존 카테고리와 동시에 노출되어야 하는 요구가 생기면 그때 프로모션 또는 태그 모델을 별도로 검토한다.

`MenuService`는 메뉴와 필요한 카테고리를 각각 한 번씩 일괄 조회하므로 메뉴 개수만큼 카테고리 쿼리가 늘어나는 N+1이 발생하지 않는다. 카테고리의 `display_order`와 메뉴 ID 순서로 정렬한다. 현재 요구사항에는 QueryDSL, EntityGraph, `@BatchSize`와 Fetch Join이 필요하지 않다.

Service는 HTTP 응답 DTO가 아닌 재사용 가능한 `MenuResult`를 반환한다. REST Controller는 `MenuResult`를 `MenuListResponse`로 변환하므로 이후 주방 API, WebSocket 또는 배치 작업도 웹 응답 타입에 의존하지 않고 같은 조회 로직을 사용할 수 있다.

### 7.2 메뉴 상세와 옵션 조회

`MenuService`는 메뉴 ID로 메뉴 존재 여부와 카테고리 참조 무결성을 검증한 뒤 해당 메뉴의 옵션을 옵션 ID 순서로 조회한다. 단일 메뉴를 조회하므로 메뉴, 카테고리와 옵션을 각각 한 번 조회하며 N+1 쿼리가 발생하지 않는다.

`MenuOption`은 사이즈와 온도를 문자열 enum으로 저장하여 enum 순서 변경으로 DB 값의 의미가 바뀌는 문제를 막는다. 원두 종류는 문자열, 추가 가격은 원화 단위 `long`, 시럽 가능 여부는 boolean으로 저장한다.

요청한 메뉴가 없으면 `MENU_NOT_FOUND`를 반환한다. 메뉴는 존재하지만 선택 옵션이 없으면 오류가 아니라 빈 옵션 목록을 반환한다.

엔티티의 상태와 저장 값 변경은 엔티티 메서드가 맡는다. Service는 요청과 현재 상태 검증, Repository 호출과 트랜잭션 조정을 담당하며 엔티티 필드를 직접 변경하지 않는다.
...
### 7.8 Transactional Outbox와 Kafka

주문 DB 커밋과 Kafka 발행은 하나의 원자적 작업이 아니다. 현재 구현은 주문 트랜잭션 커밋 후 OrderEventProducer가 Kafka 이벤트를 발행하도록 하여 롤백된 주문이 전달되지 않게 한다. 발행 실패 재처리와 유실 방지가 필요해지는 시점에는 주문과 Outbox 이벤트를 같은 트랜잭션으로 저장하는 방식으로 확장한다.
...
```text
주문 트랜잭션 커밋
→ OrderEventProducer가 Kafka order-events 토픽 발행
→ Consumer가 PROCESSED_EVENT에 eventId 원자적 등록
→ 이미 등록된 eventId라면 집계하지 않음
→ 신규 eventId라면 DAILY_PRODUCT_SALES UPSERT
→ quantity와 order_count 원자적 증가
```

현재 Kafka Consumer는 중복 가능성을 전제로 한다. 모든 이벤트에 고유한 `eventId`를 부여하고 소비자는 성공적으로 처리한 이벤트 ID를 `PROCESSED_EVENT`에 기록하여 멱등하게 동작한다. Outbox Publisher가 Kafka 발행에 실패하면 `OUTBOX_EVENT`의 재시도 횟수와 다음 재시도 시각을 갱신하고, 정한 횟수를 초과하면 `FAILED` 상태로 보존한다. Consumer의 외부 플랫폼 호출 실패는 Producer Outbox 상태와 섞지 않고 Kafka 재시도 토픽과 Dead Letter Topic으로 관리한다. 운영 환경의 고가용성을 주장하려면 단일 브로커가 아니라 다중 브로커와 복제 구성을 사용해야 한다.
...
- 메뉴 ID, 카테고리 ID·이름, 메뉴 이름과 가격 목록 반환
- 유효한 사용자 포인트 충전
